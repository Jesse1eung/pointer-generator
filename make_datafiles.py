...
all_train_urls = "/home1/liangjx/data/url_lists/all_train.txt"
all_val_urls = "/home1/liangjx/data/url_lists/all_val.txt"
all_test_urls = "/home1/liangjx/data/url_lists/all_test.txt"

cnn_tokenized_stories_dir = "/home1/liangjx/data/cnndm/cnn_stories_tokenized"
dm_tokenized_stories_dir = "/home1/liangjx/data/cnndm/dm_stories_tokenized"
finished_files_dir = "/home1/liangjx/data/phrases_finished_files"
...
def chunk_file(set_name):
    in_file = '/home1/liangjx/data/phrases_finished_files/%s.bin' % set_name
...
def get_art_abs(story_file):
    ...
    art_lens = len(article_lines)
    if art_lens <= 1:
        return "", "", ""
    random_split = art_lens // 2
    # Make article into a single string
    article1 = ' '.join(article_lines[:random_split])
    # add article2 so that we can split the original article into two snippets
    article2 = ' '.join(article_lines[random_split:])
    ###
    # Make abstract into a signle string, putting <s> and </s> tags around the sentences
    abstract = ' '.join(["%s %s %s" % (SENTENCE_START, sent, SENTENCE_END) for sent in highlights])

    return article1, article2, abstract
...
def write_to_bin(url_file, out_file, makevocab=False):
    ...
    with open(out_file, 'wb') as writer:
        for idx, s in enumerate(story_fnames):
            ...
            article_1, article_2, abstract = get_art_abs(story_file)
            if not article_1:
                empty += 1
                continue
            # Write to tf.Example
            tf_example = example_pb2.Example()
            tf_example.features.feature['article_1'].bytes_list.value.extend([bytes(article_1, encoding='utf-8')])
            tf_example.features.feature['article_2'].bytes_list.value.extend([bytes(article_2, encoding='utf-8')])
            ...
            if makevocab:
                art_tokens = article_1.split(' ')
                art_tokens += article_2.split(' ')
    ...
    print("the number of the single sentence: %d" % empty)
