...
class Example(object):
    ...
    def __init__(self, articles, abstract_sentences, vocab):
        ...
        for article in articles:
            # Process the article, decode article_words from bytes to str.
        ...
        self.original_article = articles
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list):
        ...
        self.enc_batch = np.zeros((config.num_encoders, self.batch_size, max_enc_seq_len), dtype=np.int32)
        self.enc_lens = np.zeros((config.num_encoders, self.batch_size), dtype=np.int32)
        self.enc_padding_mask = np.zeros((config.num_encoders, self.batch_size, max_enc_seq_len), dtype=np.float32)

        # Fill in the numpy arrays
        for i, ex in enumerate(example_list):
            for k in range(config.num_encoders):
                self.enc_batch[k, i, :] = ex.enc_inp[k][:]
                self.enc_lens[k, i] = ex.enc_len[k]
                self.enc_padding_mask[k, i, :ex.enc_len[k]] = 1
                # for j in range(ex.enc_len[k]):
                #     self.enc_padding_mask[k, i, j] = 1

        # For pointer-generator mode, need to store some extra info
        if config.pointer_gen:
            # Determine the max number of in-article OOVs in this batch
            # self.max_art_oovs = max([len(ex.article_oovs) for ex in example_list])
            self.max_art_oovs = max_enc_seq_len * config.num_encoders
...
class Batcher(object):
    ...
    def fill_example_queue(self):
        ...
        # modify the output of the pair_generator()
        ...
        while True:
            ...
            # modify the attribution of the Example
    ...
    def pair_generator(self, example_generator):
        while True:
            ...
            try:
                article_text1 = e.features.feature['article_1'].bytes_list.value[
                    0]  # the article text was saved under the key 'article' in the data files
                article_text2 = e.features.feature['article_2'].bytes_list.value[0]
            ...
            if len(article_text1) == 0 or len(
                ...
                yield ((article_text1, article_text2), abstract_text)
